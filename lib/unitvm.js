...
export function toggle() {
  ...
  this.serialListView.awaitSelection().then(newPort => {
    ...
    this.serialListView.setActiveTarget(newPort);

    let cwd = atom.project.getPaths()[0];
    let cfgFilePath = path.join(cwd, '.unitvm-build.yml');
    if (existsSync(cfgFilePath)) {
      cfg = yaml.load(fs.readFileSync(cfgFilePath), 'utf8');
    } else {
      atom.notifications.addSuccess("Create default '.unitvm-build.yml' file in project directory");
    }

    cfg.avrdude.port = newPort;
    fs.writeFileSync(cfgFilePath, yaml.dump(cfg), 'utf8');

    atom.commands.dispatch(atom.views.getView(atom.workspace.getActivePane()), 'build:select-active-target');
