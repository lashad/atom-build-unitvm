...
import {
    ...
    checkUnitKitPath, configExists, generateEUI64
} from "./utils";
const os = require('os');
...
export function unitkitSetHome() {
    ...
    let filePaths = remote.dialog.showOpenDialogSync(remote.BrowserWindow.getFocusedWindow(), {
        message: message,
        buttonLabel: 'Choose',
...
export function selectSerialPort() {
    ...
    if (/^darwin/.test(process.platform)) {
        ...
        arg = "/dev -maxdepth 1 -name 'cu*'";
    } else if (/^linux/.test(process.platform)) {
        emptyMessage = "No ports found (/dev/ttyA* /dev/ttyUSB*)";
        arg = "/dev -maxdepth 1 -name 'ttyA*' -o -name 'ttyUSB*'";
    }
...
export function downloadUnitKitFramework() {
    ...
    let index = remote.dialog.showMessageBoxSync(remote.BrowserWindow.getFocusedWindow(), {
