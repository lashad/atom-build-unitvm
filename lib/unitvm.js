...
export function unitkitSetHome() {
  ...
  if(filePaths !== undefined) {
    atom.config.set('build-unitvm.unitvmKitHomeDir', filePaths[0]);
  }
}
...
export function downloadUnitKitFramework() {
  ...
  let index = remote.dialog.showMessageBoxSync({message: 'Download UnitKit Framework',
                                     detail: 'Specify the folder where you want to download and install UnitKit Framework',
  ...
  if(index == 0) {
    ...
    if(filePaths !== undefined) {
        ...
        const dest = filePaths[0] + `/${kitfilename}`;
