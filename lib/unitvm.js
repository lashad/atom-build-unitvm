...
import { getCliCommand, getYamlConfig,
         writeConfig, getFiles, getUnitKitPath,
         setUnitKitPath, getProjectDirectory,
         checkUnitKitPath, configExists} from "./utils";
...
export function unitkitSetHome() {
  ...
  let filePaths = remote.dialog.showOpenDialogSync({ title: message,
                                                    message: message,
                                                    buttonLabel: 'Choose',
                                                    properties: ['openDirectory'] });
  if(filePaths !== undefined) {
    setUnitKitPath(filePaths[0]);
  }
}
...
export function downloadUnitKitFramework() {
  ...
  let index = remote.dialog.showMessageBoxSync({message: 'Download and Install UnitKit Framework',
  ...
  if(index == 0) {
    ...
    if(filePaths !== undefined) {
        ...
        statusBarItem.download(dest, src, filePaths[0]).then(() => {
          ...
          statusBarItem.setInfo(null);
          setUnitKitPath(path.join(filePaths[0], 'unitkit'));
        }).catch(() => {
  ...
  let cfg = getYamlConfig();
    ...
    statusBarItem.onClick((ev) => {
      ...
      if(checkUnitKit()) {
        ...
        if(!configExists()) {
          let index = remote.dialog.showMessageBoxSync({message: `Configuration file '${defaultConfigFileName}' does not exist in current project`,
...
export function checkUnitKit() {
  ...
  if(!checkUnitKitPath()) {
    ...
    let index = remote.dialog.showMessageBoxSync({message: 'UnitKit Framework directory is not set',
