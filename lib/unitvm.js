...
export function activate(state) {
    ...
    subscriptions.add(atom.commands.add('atom-workspace', {
        ...
        'build-unitvm:viewPinouts': (e) => this.viewPinouts(e),
...
export function addPinoutsMenuItems() {
    ...
    listFiles(getUnitKitPath(path.join('docs', 'pinouts'))).then((stdout) => {
        let names = stdout.split("\n");
        names.pop();
        names.forEach(element => {
            submenu[0].submenu.push({
                "label": element,
                "command": "build-unitvm:viewPinouts"
            });
        });
...
export function viewPinouts(e) {
    console.log(e);
    shell.beep();
    shell.openPath(getUnitKitPath(path.join('docs', 'pinouts', 'ATMega328P_pinout.png')));
}
